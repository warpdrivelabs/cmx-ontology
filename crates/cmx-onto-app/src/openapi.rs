...
pub async fn openapi_json() -> Json<Value> {
    Json(json!({
        ...
        "paths": {
            ...
            "/manifest": p("本体全量清单（?types= 子集；?include= 状态分层；?view= 场景口径）", &["get"], "建模"),
            "/lifecycle/transition": p("状态流转（七类唯一入口；dryRun 预览；兼容矩阵+机械级联）", &["post"], "建模"),
            "/revisions": p("资源修订历史（?kind=&apiName=；deleted=true 列已删除）", &["get"], "建模"),
            "/revisions/detail": p("单条修订详情（含 payload）", &["get"], "建模"),
            "/revisions/revert": p("git revert 式回滚（墓碑资源自动恢复）", &["post"], "建模"),
            "/releases": p("命名发布标记（发布门禁 + tag；发布 = 给检查点起名）", &["post"], "建模"),
            "/releases/remove": p("解除发布标记（保留检查点）", &["post"], "建模"),
            "/snapshots": p("存档：当前 live 快照 → 版本检查点（rev 去重）", &["post"], "建模"),
